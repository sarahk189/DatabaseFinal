...
public class ProductBaseRepository<TEntity> where TEntity : class
{
    private readonly ProductCatalogueContext _context;

    public ProductBaseRepository(ProductCatalogueContext context)
    {
        _context = context;
    }
    ...
    public virtual async Task<TEntity?> GetOneAsync(Expression<Func<TEntity, bool>> expression)
    {
        ...
        {
            Console.WriteLine("Error getting entity");
            Debug.WriteLine(ex.Message);
            return null!;
        }
    }
